...
use crate::app::components::{
    can_connect, Block, BlockId, BlockInstance, BlockType, BlockView, Connection, EditMode,
    InputRow, PortStatus, SourceEditorContext, SourceEditorForm, SourceEditorSidebar,
    BLOCK_HEADER_HEIGHT, BLOCK_HEIGHT, BLOCK_PORT_HEIGHT, BLOCK_WIDTH,
};
use crate::app::PlaylistContext;
use shared::model::{
    ConfigInputDto, ConfigTargetDto, HdHomeRunTargetOutputDto, M3uTargetOutputDto,
...
fn create_instance(block_type: BlockType) -> BlockInstance {
    match block_type {
        ...
        BlockType::OutputM3u => {
            BlockInstance::Output(Rc::new(TargetOutputDto::M3u(M3uTargetOutputDto::default())))
        }
        BlockType::OutputXtream => BlockInstance::Output(Rc::new(TargetOutputDto::Xtream(
            XtreamTargetOutputDto::default(),
        ))),
        BlockType::OutputHdHomeRun => BlockInstance::Output(Rc::new(TargetOutputDto::HdHomeRun(
            HdHomeRunTargetOutputDto::default(),
        ))),
        BlockType::OutputStrm => BlockInstance::Output(Rc::new(TargetOutputDto::Strm(
...
fn create_output_instance(output: &TargetOutputDto) -> (BlockInstance, BlockType) {
    match output {
        TargetOutputDto::Xtream(dto) => (
            BlockInstance::Output(Rc::new(TargetOutputDto::Xtream(dto.clone()))),
            BlockType::OutputXtream,
        ),
        TargetOutputDto::M3u(dto) => (
            BlockInstance::Output(Rc::new(TargetOutputDto::M3u(dto.clone()))),
            BlockType::OutputM3u,
        ),
        TargetOutputDto::Strm(dto) => (
            BlockInstance::Output(Rc::new(TargetOutputDto::Strm(dto.clone()))),
            BlockType::OutputStrm,
        ),
        TargetOutputDto::HdHomeRun(dto) => (
            BlockInstance::Output(Rc::new(TargetOutputDto::HdHomeRun(dto.clone()))),
...
pub fn SourceEditor() -> Html {
                ...
                for (inputs, targets) in entries.as_ref() {
                    ...
                    for target_config in targets {
                        ...
                        input_ids.iter().for_each(|input_id| {
                            gen_connections.push(Connection {
                                from: *input_id,
                                to: target_id,
                            })
                        });
                ...
                if let Some(window) = web_sys::window() {
                    if let Some(document) = window.document() {
                        for block_id in &block_ids {
                            if let Some(el) =
                                document.get_element_by_id(&format!("block-{block_id}"))
                            {
                            ...
                            let connections = {
                                ...
                                for conn in &editor_state.connections {
                                    ...
                                    {
                                        if let Some(el) = document.get_element_by_id(&format!(
                                            "conn-{}-{}",
                                            conn.from, conn.to
                                        )) {
    ...
    let handle_drag_start = {
        ...
        Callback::from(move |e: DragEvent| {
            ...
            if let Some(target) = e.target_dyn_into::<HtmlElement>() {
                ...
                e.data_transfer()
                    .unwrap()
                    .set_data("text/plain", &block_type)
                    .unwrap();
                ...
                if let Ok(data) = e.data_transfer().unwrap().get_data("text/plain") {
                    ...
                    let ((canvas_ox, canvas_oy), (offset_x, offset_y)) = {
                        ...
                        (
                            ...
                            editor_state.drag.sidebar_drag_offset,
                        )
                    };
            ...
            if let Some(canvas) = canvas_ref.cast::<HtmlElement>() {
                ...
                if let Some(block) = possible_block {
                    ...
                    let (selected_blocks, new_selection) = {
                        let is_selected =
                            editor_state.selection.selected_blocks.contains(&block_id);
                    ...
                    editor_state.drag.with_drag_block_offset(
                        block_id,
                        (mouse_x - block.position.0, mouse_y - block.position.1),
                    );
                            ...
                            if editor_state.selection.is_selecting {
                                ...
                                if let Some(rect_el) = canvas_ref.cast::<HtmlElement>() {
                                    ...
                                    if e.ctrl_key() {
                                        ...
                                        editor_state
                                            .selection
                                            .with_selecting_start_rect_and_clear_blocks(
                                                true,
                                                (mouse_x, mouse_y),
                                                Some((mouse_x, mouse_y, 0.0, 0.0)),
                                            );
    ...
    let move_blocks = {
        let editor_state_ref = editor_state_ref.clone();
        Callback::from(
            move |(mouse_x, mouse_y, offset, initial_positions): MoveBlockParams| {
                {
                    let to_collect: HashSet<BlockId> = initial_positions
                        .iter()
                ...
                if !moved_block_ids.is_empty() {
                    ...
                    for conn in &editor_state.connections {
                        if moved_block_ids.contains(&conn.from)
                            || moved_block_ids.contains(&conn.to)
                        {
                            move_connections.insert(
                    ...
                    for (from, to) in move_connections.values() {
                        if let Some(path_el) = editor_state.connection_elements.get(&(*from, *to)) {
                            ...
                            {
                                let (d, _) =
                                    update_connection(canvas_ox, canvas_oy, from_block, to_block);
        ...
        Callback::from(move |e: MouseEvent| {
            ...
            *last_frame.borrow_mut() = now;
            ...
            if let Some(canvas) = canvas_ref.cast::<HtmlElement>() {
                ...
                if is_selecting {
                    let (x, y, w, h) =
                        compute_normalized_selection_rect(selection_start, mouse_x, mouse_y);
                    let ctrl_key = e.ctrl_key();
                            ...
                            if let Some(window) = web_sys::window() {
                                if let Some(document) = window.document() {
                                    if let Some(el) = document.get_element_by_id(SELECTION_RECT) {
                                        editor_state.selection.select_rect_elem =
    ...
    let form_changed = {
        let editor_state_ref = editor_state_ref.clone();
        Callback::<(BlockId, BlockInstance)>::from(
            move |(block_id, instance): (BlockId, BlockInstance)| {
    ...
    let ((canvas_off_x, canvas_off_y), select_rect_style, pending_line) = {
        ...
        let select_rect_style = editor_state
            .selection
            .selection_rect
            .as_ref()
            .map(|(x, y, w, h)| format!("left:{x}px; top:{y}px; width:{w}px; height:{h}px;"));
...
fn compute_normalized_selection_rect(
    selection_start: Position,
    mouse_x: f32,
    mouse_y: f32,
) -> (f32, f32, f32, f32) {
