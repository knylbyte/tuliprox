...
use std::rc::Rc;
use yew::{function_component, html, use_context, use_effect_with, use_reducer, Callback, Html, UseReducerHandle};
...
use crate::{config_field_child, edit_field_bool, edit_field_list, edit_field_number_i16, edit_field_number_u16,
...
pub fn ConfigInputView() -> Html {
    ...
    let input_form_state: UseReducerHandle<ConfigInputFormState> = use_reducer(|| {
        ConfigInputFormState { form: ConfigInputDto::default(), modified: false }
    });
    let input_options_state: UseReducerHandle<ConfigInputOptionsDtoFormState> = use_reducer(|| {
        ConfigInputOptionsDtoFormState { form: ConfigInputOptionsDto::default(), modified: false }
    });
    let staged_input_state: UseReducerHandle<StagedInputDtoFormState> = use_reducer(|| {
        StagedInputDtoFormState { form: StagedInputDto::default(), modified: false }
    });

    {
        ...
        use_effect_with(
            ...
            move |(input_form, input_options, staged_input)| {
                ...
                form.options = if input_options.form.is_empty() { None} else { Some(input_options.form.clone()) };
                form.staged = if staged_input.form.is_empty() { None} else { Some(staged_input.form.clone()) };

                let modified = input_form.modified
                    || input_options.modified
                    || staged_input.modified;
                on_form_change.emit(**input_form);
            },
        );
    }

    {
        ...
        let config_input = source_editor_ctx.input.as_ref();
        use_effect_with(config_input, move |cfg| {
            if let Some(input) = cfg {
                ...
                input_options_state.dispatch(ConfigInputOptionsFormAction::SetAll(input.options.as_ref().map_or_else(ConfigInputOptionsDto::default, |d| d.clone())));
                staged_input_state.dispatch(StagedInputFormAction::SetAll(input.staged.as_ref().map_or_else(StagedInputDto::default, |c| c.clone())));
            } else {
    ...
    let render_input = || html! {
         <Card class="tp__config-view__card">
           ...
           { config_field_child!(translate.t(LABEL_FETCH_METHOD), {
               let method_selection = Rc::new(vec![input_form_state.form.method.to_string()]);
               html! {
                   <RadioButtonGroup
                    ...
                    on_select={Callback::from(move |selections: Rc<Vec<String>>| {
                        ...
                        input_form_state.dispatch(ConfigInputFormAction::Method(first.parse::<InputFetchMethod>().unwrap_or(InputFetchMethod::GET)));
                    })}
                    options={[InputFetchMethod::GET, InputFetchMethod::POST].iter().map(ToString::to_string)}
                    selected={method_selection}
                />
           }})}
           { config_field_child!(translate.t(LABEL_INPUT_TYPE), {
               let input_type_selection = Rc::new(vec![input_form_state.form.input_type.to_string()]);
               html! {
                   <RadioButtonGroup
                    multi_select={false} none_allowed={false}
                    on_select={Callback::from(move |selections: Rc<Vec<String>>| {
                    ...
                    options={[InputType::M3u, InputType::Xtream, InputType::M3uBatch, InputType::XtreamBatch].iter().map(ToString::to_string)}
                    selected={input_type_selection}
                />
           }})}
