...
use crate::{
    config_field, config_field_bool, config_field_custom, config_field_hide, config_field_optional,
...
use shared::model::{
    CacheConfigDto, GeoIpConfigDto, RateLimitConfigDto, ResourceRetryConfigDto,
    ReverseProxyConfigDto, ReverseProxyDisabledHeaderConfigDto, StreamConfigDto,
};
...
pub fn ReverseProxyConfigView() -> Html {
    ...
    let reverse_proxy_state: UseReducerHandle<ReverseProxyConfigFormState> =
        use_reducer(|| ReverseProxyConfigFormState {
            form: ReverseProxyConfigDto {
                rewrite_secret: default_secret(),
                ..Default::default()
            },
            modified: false,
        });
    ...
    let resource_retry_state: UseReducerHandle<ResourceRetryConfigFormState> =
        use_reducer(|| ResourceRetryConfigFormState {
            form: ResourceRetryConfigDto::default(),
            modified: false,
        });
    ...
    let geoip_state: UseReducerHandle<GeoIpConfigFormState> =
        use_reducer(|| GeoIpConfigFormState {
            form: GeoIpConfigDto::default(),
            modified: false,
        });
        ...
        use_effect_with(
            ...
            move |(cfg, _mode)| {
                if let Some(rp) = cfg {
                    ...
                    disabled_header_state.dispatch(
                        ReverseProxyDisabledHeaderConfigFormAction::SetAll(
                            rp.disabled_header
                                .as_ref()
                                .map_or_else(ReverseProxyDisabledHeaderConfigDto::default, |d| {
                                    d.clone()
                                }),
                        ),
                    );
                    ...
                    resource_retry_state.dispatch(ResourceRetryConfigFormAction::SetAll(
                        rp.resource_retry
                            .as_ref()
                            .map_or_else(ResourceRetryConfigDto::default, |rr| rr.clone()),
                    ));
                    ...
                    geoip_state.dispatch(GeoIpConfigFormAction::SetAll(
                        rp.geoip
                            .as_ref()
                            .map_or_else(GeoIpConfigDto::default, |s| s.clone()),
                    ));
                } else {
                    ...
                    resource_retry_state.dispatch(ResourceRetryConfigFormAction::SetAll(
                        ResourceRetryConfigDto::default(),
                    ));
                    stream_state
