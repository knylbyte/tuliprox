...
pub fn update_config(config: &mut ConfigDto, forms: Vec<ConfigForm>) {
    for form in forms {
        match form {
            ...
            ConfigForm::Messaging(_, mut messaging_cfg) => {
            ...
            ConfigForm::HdHomerun(_, mut hdhr_cfg) => {
                set_config_field!(config, hdhr_cfg, hdhomerun)
            }
            ConfigForm::Proxy(_, mut proxy_cfg) => set_config_field!(config, proxy_cfg, proxy),
            ConfigForm::IpCheck(_, mut ipcheck_cfg) => {
                set_config_field!(config, ipcheck_cfg, ipcheck)
            }
