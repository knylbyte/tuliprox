...
pub fn LogConfigView() -> Html {
    ...
    let form_state: UseReducerHandle<LogConfigFormState> = use_reducer(|| LogConfigFormState {
        form: LogConfigDto::default(),
        modified: false,
    });
...

    {
        ...
        use_effect_with(
            (log_config, config_view_ctx.edit_mode.clone()),
            move |(log_cfg, _mode)| {
    ...
    let render_edit_mode = || {
        ...
        let log_level_selection = Rc::new(
            forms
                .form
                .log_level
                .as_ref()
                .map_or_else(Vec::new, |l| vec![l.to_uppercase()]),
        );
        html! {
            <>
