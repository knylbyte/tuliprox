...
use crate::app::components::{
    convert_bool_to_chip_style, AppIcon, BatchInputContentView, Chip, EpgConfigView, HideContent,
    InputHeaders, InputOptions, InputTypeView, RevealContent, StagedInputView, Table,
    TableDefinition,
};
use crate::html_if;
...
pub fn InputTable(props: &InputTableProps) -> Html {
    ...
    let render_data_cell = {
        ...
        Callback::<(usize, usize, Rc<InputRow>), Html>::from(
            move |(row, col, input): (usize, usize, Rc<InputRow>)| match &*input {
                InputRow::Input(dto) => match col {
                    ...
                    4 => {
                        html! { if matches!(dto.input_type, InputType::XtreamBatch | InputType::M3uBatch) {
                    ...
                    5 => dto
                        .username
                        .as_ref()
                        .map_or_else(|| html! {}, |u| html! {u}),
                    6 => dto.password.as_ref().map_or_else(
                        || html! {},
                        |pwd| html! { <HideContent content={pwd.to_string()}></HideContent>},
                    ),
                ...
                InputRow::Alias(alias, dto) => match col {
                    0 => {
                        let popup_onclick = popup_onclick.clone();
                        html! {
                            <button class="tp__icon-button"
                    ...
                    5 => alias
                        ...
                        .as_ref()
                        .map_or_else(|| html! {}, |u| html! {u}),
                    6 => alias.password.as_ref().map_or_else(
                        || html! {},
                        |pwd| html! { <HideContent content={pwd.to_string()}></HideContent>},
                    ),
