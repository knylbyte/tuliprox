...
impl TmdbClient {
    ...
    //     }
    //
    //     let series_content = response.bytes().await.map_err(|e| format!("Failed to read TMDB response body: {e}"))?;
    //
    //     self.fetched_series_metadata.write().await.insert(series_id);
    //
    //     let mut series = serde_json::from_slice::<TmdbSeriesInfoDetails>(&series_content)
    //         .map_err(|e| format!("Failed to parse TMDB series details: {e}"))?;
    //
    //     let season_count = self.detect_season_count(&series);
    //     if season_count > 0 {
    ...
    //         if !season_infos.is_empty() {
    //
    //             let raw_series = serde_json::from_slice::<serde_json::Map<String, serde_json::Value>>(&series_content);
    //
    //             if let Some(seasons) = series.seasons.as_mut() {
    //                 for season in seasons {
    //                     let season_no = season.season_number;
    //                     for (season_details, raw_season_content) in &season_infos {
    //                         if season_details.season_number == season_no {
    //                             season.episodes = Some(season_details.episodes.clone());
    //                             season.networks = Some(season_details.networks.clone());
    //                             season.credits = season_details.credits.clone();
    //
    //                             if let Ok(series_json) = raw_series {
    //                                 if let Ok(series_season_details) = serde_json::from_slice::<serde_json::Map<String, serde_json::Value>>(raw_season_content.as_ref()) {
    //                                     if let Some(season_entry) = series_json.get("seasons") {
    //                                         match season_entry {
    //                                             Value::Array(season_entry_list) => {
    //                                                 for season_item in season_entry_list {
    //                                                     if let Some(season_number) = season_item.get("season_number") {
    //                                                         match season_number {
    //                                                             Value::Number(season_item_no) => {
    //                                                                 if let Some(no) = season_item_no.as_u64() {
    //                                                                     if season_no as u64 == no {
    //                                                                         season_item.insert("episodes", series_season_details.get("episodes"));
    //                                                                         season_item.insert("networks", series_season_details.get("networks"));
    //                                                                         season_item.insert("credits", series_season_details.get("credits"));
    //                                                                     }
    //                                                                 }
    //                                                             }
    //                                                             _ => {}
    ...
    //             }
    //
    //             if let Ok(raw_series_content) = &raw_series {
    //                 if let Ok(series_content) = serde_json::to_vec(raw_series_content) {
    //                     if let Err(err) = self.storage.store_tmdb_series_info(series_id, &series_content).await {
    //                         error!("Failed to store raw TMDB series info: {err}");
    //                     }
    //                 }
    //             }
