...
use crate::library::scanner::{ScannedVideoFile, VodScanner};
...
impl LibraryProcessor {
    ...
    pub fn new(config: VodConfig) -> Self {
        let storage_path = std::path::PathBuf::from(&config.metadata.storage.location);
    ...
    pub async fn scan(&self, force_rescan: bool) -> Result<LibraryScanResult, std::io::Error> {
        ...
        *self.next_virtual_id.write().await = max_virtual_id + 1;
        ...
        let scanned_paths: std::collections::HashSet<_> = scanned_files.iter().map(|f| f.path.clone()).collect();
    ...
    async fn write_metadata_files(&self, entry: &MetadataCacheEntry) -> Result<(), std::io::Error> {
        ...
        if self.config.metadata.storage.write_nfo {
