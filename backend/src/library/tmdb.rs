...
use crate::library::{ExternalVideoMetadata};
...
impl TmdbVideo {
    pub fn to_meta_data(&self) -> ExternalVideoMetadata {
        ExternalVideoMetadata {
...
impl TmdbMovieDetails {
    pub(crate) fn to_meta_data(&self) -> MovieMetadata {
        MovieMetadata {
            ...
            tvdb_id: self.external_ids.as_ref().and_then(|ei| ei.tvdb_id.clone()),
            ...
            videos: self.videos
                .as_ref()
                .and_then(|v| v.get_youtube())
                .map(|list| {
                    list.iter()
                        .map(|v| v.to_meta_data())
                        .collect::<Vec<_>>()
                }).filter(|v| !v.is_empty()),
        }
    }
...
pub struct TmdbSeriesInfoDetails {
    ...
    external_ids: Option<TmdbExternalIds>,
    videos: Option<TmdbVideos>,
    pub(crate) episodes: Option<Vec<TmdbSeriesInfoSeasonDetails>>,
}
