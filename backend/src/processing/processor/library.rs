...
use shared::utils::string_to_uuid;
use std::sync::Arc;
use std::path::Path;
...
fn to_playlist_item(entry: &MetadataCacheEntry, input_name: &str, group_name: &str) -> PlaylistItem {
    ...
    PlaylistItem {
        header: PlaylistItemHeader {
            uuid: string_to_uuid(&entry.uuid),
            id: String::new(),
            virtual_id: 0,
...
pub fn metadata_cache_entry_to_xtream_movie_info(
    ...
    let info = XtreamMovieInfoDetails {
        ...
        actors: movie.actors.as_ref().map(|a| {
            a.iter().map(|a| a.name.clone()).collect::<Vec<_>>().join(", ")
        }),
        cast: movie.actors.as_ref().map(|a| {
            a.iter().map(|a| a.name.clone()).collect::<Vec<_>>().join(", ")
        }),
