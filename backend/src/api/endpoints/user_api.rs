...
use crate::repository::{iter_raw_m3u_playlist, load_user_bouquet_as_json, save_user_bouquet, xtream_get_playlist_categories};
...
async fn get_categories_from_m3u_playlist(target: &ConfigTarget, config: &AppConfig) -> Vec<Arc<str>> {
    ...
    if let Some((_guard, iter)) = iter_raw_m3u_playlist(config, target).await {
