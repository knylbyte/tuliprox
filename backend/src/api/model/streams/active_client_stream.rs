...
pub(in crate::api) struct ActiveClientStream {
    ...
    custom_video: (Option<TransportStreamBuffer>, Option<TransportStreamBuffer>, Option<TransportStreamBuffer>, Option<TransportStreamBuffer>),
...
impl ActiveClientStream {
    ...
    pub(crate) async fn new(mut stream_details: StreamDetails,
        ...
        let provisioning_info = Self::resolve_grace_period_provisioning(app_state, &stream_details);
        let (grace_stop_flag, waker) = if grant_user_grace_period || (stream_details.has_grace_period() && stream_details.provider_name.is_some()) {
            ...
            let flag = Self::stream_grace_period(app_state, &stream_details, grant_user_grace_period, user, fingerprint, virtual_id, provisioning_info, Some(Arc::clone(&waker)));
            let maybe_waker = flag.as_ref().map(|_| waker);
            (flag, maybe_waker)
        } else {
            (Self::stream_grace_period(app_state, &stream_details, grant_user_grace_period, user, fingerprint, virtual_id, provisioning_info, None), None)
        };
                    ...
                    if let Some(provider_name) = provider_grace_check {
                        if provider_manager.is_over_limit(&provider_name).await {
                            if let Some(provisioning_info) = provisioning_info {
                                ...
                                connection_manager.update_stream_detail(&fingerprint.addr,CustomVideoStreamType::Provisioning).await;
                                debug_if_enabled!("Provider grace period exhausted; provisioning for active clients: {provider_name}");
                                let app_state = Arc::clone(&app_state);
                                let input = (*provisioning_info.input).clone();
                                let stop_signal = provisioning_info.stop_signal;
                                let addr = fingerprint.addr;
                                tokio::spawn(async move {
                                    run_panel_api_provisioning_probe(app_state, input, stop_signal, addr, virtual_id).await;
                                });
                            } else {
...
impl Stream for ActiveClientStream {
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut std::task::Context<'_>) -> Poll<Option<Self::Item>> {
        ...
        let buffer_opt = match flag {
            ...
            CHANNEL_UNAVAILABLE_STREAM => self.custom_video.2.as_mut(),
            PROVISIONING_STREAM => self.custom_video.3.as_mut(),
            _ => None,
        };
