...
mod packet {
    ...
    pub const HDHOMERUN_DEVICE_TYPE_TUNER: u32 = 0x00000001;
    pub const HDHOMERUN_DEVICE_ID_WILDCARD: u32 = 0xFFFFFFFF;
}
...
fn parse_tlv(cursor: &mut Cursor<&[u8]>) -> HashMap<u8, Vec<u8>> {
    ...
    while cursor.position() < cursor.get_ref().len() as u64 {
        ...
        if cursor.get_ref().len() < (cursor.position() as usize) + len {
            break;
        }
...
async fn proprietary_discover_loop(
    ...
    loop {
        ...
        if msg_type == packet::HDHOMERUN_TYPE_DISCOVER_REQ {
            ...
            let is_discover_all = tags
                .get(&packet::HDHOMERUN_TAG_DEVICE_ID)
                .map_or(false, |id| {
                    u32::from_be_bytes(id.clone().try_into().unwrap_or_default())
                        == packet::HDHOMERUN_DEVICE_ID_WILDCARD
                });

            if is_discover_all {
                trace!(
                    "Received proprietary HDHomeRun M-SEARCH from {}",
                    remote_addr
                );
                        ...
                        for device in &hd_config.devices {
                            if device.t_enabled {
                                ...
                                if let Err(e) = socket.send_to(&response, remote_addr).await {
                                    error!(
                                        "Failed to send proprietary SSDP response to {}: {}",
                                        remote_addr, e
                                    );
                                } else {
                                    trace!(
                                        "Sent proprietary SSDP response for device '{}' to {}",
                                        device.name,
                                        remote_addr
                                    );
                                }
...
async fn process_getset_request(request: &[u8], app_state: &Arc<AppState>) -> Vec<u8> {
    ...
    if let Some(name_bytes) = tags.get(&packet::HDHOMERUN_TAG_GETSET_NAME) {
        ...
        match name_str {
            ...
            s if s.starts_with("/tuner") && s.ends_with("/status") => {
                let tuner_index_str = &s[6..s.find('/').unwrap_or(6)];
                if let Ok(tuner_index) = tuner_index_str.parse::<usize>() {
    ...
    response.put_u16(response_payload.len() as u16);
        ...
        match UdpSocket::bind(addr).await {
            Ok(socket) => {
                if let Err(e) = socket.set_broadcast(true) {
                    error!(
                        "Failed to set broadcast on proprietary HDHR socket: {}",
                        e
                    );
                    return;
                }
                ...
                tokio::select! {
                    _ = proprietary_discover_loop(socket, app_config, server_host) => {},
                    _ = cancel_token_udp.cancelled() => {
