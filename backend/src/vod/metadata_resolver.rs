...
use crate::vod::classifier::{VideoClassification, VodClassifier};
use crate::vod::metadata::{MetadataSource, MovieMetadata, SeriesMetadata, VideoMetadata};
use crate::vod::nfo_reader::NfoReader;
use crate::vod::scanner::ScannedVideoFile;
use crate::vod::tmdb_client::TmdbClient;
...
impl MetadataResolver {
    ...
    pub fn from_config(config: &VodConfig) -> Self {
        let tmdb_client = if config.metadata.tmdb_enabled {
            config.metadata.tmdb_api_key.as_ref().map(|api_key| {
                TmdbClient::new(api_key.clone(), config.metadata.tmdb_rate_limit_ms)
            })
        } else {
            None
        };
        ...
        Self {
            ...
            tmdb_client,
            fallback_to_filename: config.metadata.fallback_to_filename_parsing,
        }
    }
