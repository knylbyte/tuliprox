...
use crate::vod::metadata::{
    Actor, EpisodeMetadata, MetadataSource, MovieMetadata, SeriesMetadata, VideoMetadata,
};
...
impl NfoReader {
    ...
    pub async fn read_metadata(video_path: &Path) -> Option<VideoMetadata> {
        ...
        let nfo_candidates = vec![
            parent_dir.join(format!("{}.nfo", file_stem)), // filename.nfo
        ...
        for nfo_path in nfo_candidates {
            if fs::try_exists(&nfo_path).await.unwrap_or(false) {
                ...
                if let Ok(content) = fs::read_to_string(&nfo_path).await {
                    if let Some(metadata) = Self::parse_nfo(&content).await {
    ...
    async fn parse_nfo(content: &str) -> Option<VideoMetadata> {
        ...
        if is_movie {
            Self::parse_movie_nfo(content).await
        } else if is_series {
            Self::parse_series_nfo(content).await
        } else {
    ...
    async fn parse_movie_nfo(content: &str) -> Option<VideoMetadata> {
        let mut reader = Reader::from_str(content);
        reader.config_mut().trim_text(true);
            ...
            match reader.read_event_into(&mut buf) {
                ...
                Ok(Event::End(e)) => {
                    ...
                    match tag_name.as_str() {
                        "title" if !in_actor => movie.title = current_text.clone(),
                        ...
                        "name" if in_actor => current_actor.name = current_text.clone(),
    ...
    async fn parse_series_nfo(content: &str) -> Option<VideoMetadata> {
        let mut reader = Reader::from_str(content);
        reader.config_mut().trim_text(true);
        ...
        loop {
            match reader.read_event_into(&mut buf) {
                ...
                Ok(Event::End(e)) => {
                    let tag_name = String::from_utf8_lossy(e.name().as_ref()).to_string();
                    match tag_name.as_str() {
                        "title" if !in_actor => series.title = current_text.clone(),
...
<movie>
    ...
    <studio>Warner Bros.</studio>
        ...
        let metadata = NfoReader::parse_movie_nfo(nfo_content).await;
...
<tvshow>
    ...
    <status>Ended</status>
        ...
        let metadata = NfoReader::parse_series_nfo(nfo_content).await;
        assert!(metadata.is_some());
