...
use crate::info_err;
pub use crate::model::{
    FieldGetAccessor, FieldSetAccessor, ItemField, PatternTemplate, PlaylistItem, PlaylistItemType,
    TemplateValue,
};
...
fn get_caption<'a>(
    provider: &'a ValueProvider<'a>,
    rewc: &'a CompiledRegex,
) -> (bool, Cow<'a, str>) {
            ...
            Self::FieldComparison(field, rewc) => {
                ...
                if log_enabled!(Level::Trace) {
                    if is_match {
                        ...
                        trace!(
                            "Match failed: {self}: {rewc:?} {} => {field}='{value}'",
                            &rewc.restr
                        );
                    }
                }
    ...
    item_type.map_or_else(
        || {
            create_tuliprox_error_result!(
                TuliproxErrorKind::Info,
                "cant parse item type: {text_item_type}"
            )
        },
...
fn get_parser_expression(
    ...
    if stmts.len() > 1 {
        return Err(format!(
            "did not expect multiple rule: {stmts:?}, {errors:?}"
        ));
    }
    ...
    for template in templates {
        ...
        match &template.value {
            ...
            TemplateValue::Multi(values) => {
                values.iter().for_each(|value| handle_template_value(value))
            }
        }
    }
                ...
                if let Some(depends_on) = dependencies.get(&template_name) {
                    ...
                    for dep_templ_name in depends_on {
                        let dep_value = template_values.get(dep_templ_name).ok_or_else(|| {
                            info_err!(format!("Failed to load template {dep_templ_name}"))
                        })?;
                                    ...
                                    match templ_value {
                                        TemplateValue::Single(templ_val) => {
                                            if templ_val.contains(&dep_templ.placeholder) {
                                                TemplateValue::Single(templ_val.replace(
                                                    &dep_templ.placeholder,
                                                    dep_val.as_str(),
                                                ))
                                            } else {
                                                TemplateValue::Single(templ_val)
                                ...
                                TemplateValue::Multi(dep_vals) => match templ_value {
                                    TemplateValue::Single(templ_val) => {
                                        ...
                                        for dep_val in dep_vals {
                                            if templ_val.contains(&dep_templ.placeholder) {
                                                new_values.push(templ_val.replace(
                                                    &dep_templ.placeholder,
                                                    dep_val.as_str(),
                                                ));
                                            } else {
...
pub fn apply_templates_to_pattern_single(
    ...
    match apply_templates_to_pattern(pattern, templates, false)? {
        ...
        TemplateValue::Multi(_) => create_tuliprox_error_result!(
            TuliproxErrorKind::Info,
            "Multi value templates are not supported for pattern!"
        ),
    }
}
