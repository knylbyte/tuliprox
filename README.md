...
- `alias_pool.size.min`: `number` or `auto`.
  - `number`: keep at least this many valid (not expired) accounts on boot/update. Must be greater `0` and <= `max` (when `max` is a number).
...
- `panel_api.provisioning.offset`: optional pre-expiry window for boot/update renewal of accounts (root input account and aliases) with `exp_date`; if `now + offset > exp_date`, tuliprox tries `client_renew`, and falls back to `client_new` if renew fails. Supports suffixes `s` (seconds), `m` (minutes), `h` (hours), `d` (days), e.g. `30m`, `12h`, `2d`.
