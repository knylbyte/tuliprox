    ...
    if let Some(re) = regex {
        return if let Some(caps) = re.captures(&text) {
            if let Some(m) = caps.get(1) {
                ...
                Err(TuliproxError::new(TuliproxErrorKind::Info, "Regex matched but no group found".to_string()))
            }
        } else {
            Err(TuliproxError::new(TuliproxErrorKind::Info, "Regex did not match".to_string()))
        };
    }
